...
(empty($_POST['department']))||(empty($_POST['sdate']))||(empty($_POST['ddate']))||empty($_POST['description'])){
include('query2.php');
include('anu.html');
}
else{
...
echo "<br>File $fileName uploaded<br>";}else{echo "<br>File $fileName not uploaded<br>";}
} global $pass;
...
if($dep=='Airforce'){$airforce='Yes';}
  $sql6="select * from allusers where Name='$cby' && QArmy='Yes' && QNavy='Yes' && QAirforce='Yes'";
  ...
  if($row<=0){
    if($rowc<=0){
     ...
     alert('Createdby Name doesnot exist in Super Users!');
     </script>";
    }else{
    if($rowa<=0){
    include('query2.php');
  ...
  $result1=mysql_query($sql1,$conn) or die(mysql_error());
   while($r=mysql_fetch_array($result6,MYSQL_ASSOC)){
     $pass=$r['Password'];
   }
...
alert('Sucessfully Posted!')
   ...
   echo "<form action='superuser1.php' method='post'>
      <input type='hidden' name='name' value='$cby'>
